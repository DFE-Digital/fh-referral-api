...
public class MinimalReferralEndPoints
{
    public void RegisterReferralEndPoints(WebApplication app)
    {
        ...
        app.MapGet("api/referral/compositekeys", [Authorize] async (long? serviceId, long? statusId, long? recipientId, long? referralId, CancellationToken cancellationToken, ISender _mediator) =>
        {
            GetReferralByServiceIdStatusIdRecipientIdReferrerIdCommand request = new(serviceId, statusId, recipientId, referralId);
            var result = await _mediator.Send(request, cancellationToken);
            return result;
